...
from flask import (
    ...
    redirect,
    url_for,
    jsonify,  # Adicionei jsonify para retornar respostas JSON
)
import logging
import os
import asyncio
...
# Configuration
MAIGRET_DB_FILE = str(Path(__file__).resolve().parent.parent / 'resources' / 'data.json')
...
def api_search():
    ...
    # Configuração mínima equivalente ao comando CLI
    options = {
        'top_sites': 500,
        'timeout': 30,
        'all_sites': False,
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(maigret_search(username, options))
        ...
        return jsonify({'username': username, 'results': simplified_results})
